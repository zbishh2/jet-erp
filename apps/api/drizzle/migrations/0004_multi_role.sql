...
CREATE UNIQUE INDEX IF NOT EXISTS user_org_module_role_idx ON user_organization_module (user_id, organization_id, module_id, role);

-- Step 3: For existing ESTIMATOR users, insert a VIEWER row
INSERT OR IGNORE INTO user_organization_module (id, user_id, organization_id, module_id, role, is_active, granted_at, granted_by_user_id, created_at, updated_at)
SELECT
